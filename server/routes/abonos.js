...
import mongoose from "mongoose"; // IMPORTANTE: Agregamos mongoose aqu�
...
router.post("/", async (req, res) => {
    try {
        ...
        // 1. GUARDAMOS EL RECIBO HIST�RICO SIEMPRE
        ...
        if (pagosDelAlumno && pagosDelAlumno.length > 0) {
            ...
            // Ordenamos cronol�gicamente para pagar lo m�s viejo primero
            ...
            // ----------------------------------------------------------------
            // 4. CREAR MESES FUTUROS SI A�N SOBRA DINERO
            // ----------------------------------------------------------------
            // Filtramos cursos que tengan cobro activo. Si todos est�n en $0, usamos el primero por defecto.
            ...
            while (dineroEnMano > 0) {
                ...
                // Buscamos qu� curso se qued� m�s atrasado para inyectarle el mes
            ...
            for (let pago of pagosDelAlumno) {
                ...
                // Le avisamos a Mongoose expl�citamente que los meses cambiaron para que no los borre
        ...
        res.status(201).json({
            message: "Pago distribuido en cascada con �xito",
            data: nuevoAbono
        });
