...
import { Navbar } from './Navbar'; // 👈 1. IMPORTAMOS LA NAVBAR AQUÍ
...
export function Dashboard() {
    ...
    const handleEliminarReagendacion = async (classData: CalendarClass) => {
        try {
            const idAEliminar = classData.reagendacionId || classData.idGrupo || classData.id;
            if (!idAEliminar) {
                alert('No se encontró la reagendación a eliminar');
                return;
            }
            const confirmado = window.confirm(`¿Seguro que deseas eliminar esta reagendación de "${classData.title}"?`);
            ...
            try {
                            ...
                            const studentsFiltrados = (item.alumnos || [])
                                ...
                                .map((alumno: any) => {
                                    ...
                                    if (mismaFechaOriginal) {
                                        const fechaNueva = reag.fechaHoraNueva ? parseFechaFlexible(reag.fechaHoraNueva) : null;
                                        return {
                                            ...alumno,
                                            reagendacion: {
                                                tipo: 'origen',
                                                texto: fechaNueva
                                                    ? `Nuevo horario: ${fechaNueva.toLocaleDateString('es-MX', { weekday: 'short' })} ${reag.horaClaseNueva || fechaNueva.toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit', hour12: false })}`
                                                    : '',
                                            },
                                        };
                                    }
                ...
                reagendaciones.forEach((r: any, index: number) => {
                            ...
                            (r.alumnos || []).forEach((alumno: any) => {
                                ...
                                if (!yaExiste) {
                                    existente.students.push({
                                        ...
                                        reagendacion: {
                                            tipo: 'destino',
                                            texto: fechaOriginal ? `Viene de: ${fechaOriginal.toLocaleDateString('es-MX', { weekday: 'short' })} ${alumno.reagendacion?.horaClaseOriginal || fechaOriginal.toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit', hour12: false })}` : '',
                                        },
                                    });
                                }
                            });
                    ...
                    transformedReagendaciones.push({
                        ...
                        teacher: { name: r.nombreProfesor || '', email: '' },
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-cyan-50">
            ...
            {/* 👇 2. INYECTAMOS LA NAVBAR HASTA ARRIBA DE LA PÁGINA 👇 */}
            <Navbar />

            {/* ⚠️ CAMBIO CRUCIAL: Reemplazamos 'sticky' por 'relative' para evitar superposiciones con la Navbar */}
